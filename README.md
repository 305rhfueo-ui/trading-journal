...
스토어에 올라간 앱이 아니라, 브라우저에서 바로 설치하는 방식(PWA)이다.

- **PC (Chrome / Edge)** — 사이트를 열고 주소창 오른쪽 끝의 설치 아이콘(⊕ 또는 모니터 모양) 클릭.
  안 보이면 ⋮ 메뉴 → *앱으로 설치* / *Install as app*. 시작 메뉴와 바탕화면에 아이콘이 생기고 독립 창으로 열린다.
- **아이폰 (Safari)** — 공유 버튼 → *홈 화면에 추가*.
- **안드로이드 (Chrome)** — ⋮ → *앱 설치* / *홈 화면에 추가*.

설치해도 알맹이는 같은 페이지라 업데이트가 자동으로 반영된다. 오프라인에서는 마지막으로 본 화면이 뜨고, 기록은 온라인일 때 저장된다.
...
- **목표가 · 손절가** — 진입가 아래 칸 (선택). 표에 진입가 대비 %와 계획 손익비(예: `1:2.1`)가 표시된다. 롱은 손절 < 진입 < 목표, 숏은 반대여야 저장된다.
- **평균 단가** — 진입가/청산가 칸 옆 `평균 계산` 버튼. 나눠서 체결된 수량·단가를 줄마다 넣으면 가중평균이 나오고, `폼에 넣기`를 누르면 진입가와 수량이 자동으로 채워진다.
